...
for dim in [10,20,30,40]:
    for n_samples in [30,50,100,600]:#,
        ...
        print(classification_report(int_y, pred1, target_names=targetnames))
        fig, ax = plt.subplots(figsize=(14, 14))
        cm = confusion_matrix(int_y, pred1)
        disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=targetnames, ax=ax)
        disp.plot() 
        ...
        print(classification_report(int_y, pred2, target_names=targetnames))
        fig, ax = plt.subplots(figsize=(14, 14))
        cm = confusion_matrix(int_y, pred1)
        disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=targetnames, ax=ax)
        disp.plot()  
